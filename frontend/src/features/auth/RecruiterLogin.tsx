...
import {
    Briefcase, Lock, Mail, Eye, EyeOff, ArrowRight,
...
export default function RecruiterLogin() {
    ...
    useEffect(() => {
        if (isAuthenticated && user) {
            if (user.role === "recruiter") navigate("/jobs/my", { replace: true });
    ...
    const handleLogin = async (e: React.FormEvent) => {
        ...
        setLoading(true);
        try {
            const { data } = await api.post<{ token: string; user: User; redirectTo?: string; requiresVerification?: boolean; email?: string; message?: string }>(
                "/auth/login", { email: email.toLowerCase().trim(), password });


            if (data.user?.role !== "recruiter") {
                toast.error("Access denied. This portal is for recruiters only.");
                return;
            }
            login(data.token, data.user);
            toast.success(`Welcome, ${data.user.name}! 🏢`);
            navigate(data.redirectTo || "/jobs/my", { replace: true });
        } catch (err: any) {
    ...
    const handleGoogle = async (cr: CredentialResponse) => {
        ...
        try {
            const { data } = await api.post<{ token: string; user: User; redirectTo?: string }>(
                "/auth/google", { idToken: cr.credential, role: "recruiter" });
            if (data.user?.role !== "recruiter") {
                toast.error("Access denied. This portal is for recruiters only.");
                return;
            }
            ...
            >
                <div className="apple-card p-10">
                    ...
                    <div className="flex flex-col items-center mb-10">
                        ...
                        <h1 className="text-3xl font-bold text-apple-gray-900 tracking-tight mb-2">Recruiter Portal</h1>
                        <p className="text-apple-gray-400 font-medium text-center">Manage your hiring pipeline</p>
                    </div>
                    ...
                    <form onSubmit={handleLogin} className="space-y-6">
                        <div className="space-y-2">
                            <label className="text-[13px] font-semibold text-apple-gray-400 ml-1">Corporate Email</label>
                    ...
                    <div className="mt-10 text-center">
                        <p className="text-apple-gray-400 text-sm">
                            New recruiter?{" "}
                            <Link to="/register?role=recruiter" className="text-apple-blue font-semibold hover:underline">
                                Request Access
                            </Link>
                        </p>
                    </div>
