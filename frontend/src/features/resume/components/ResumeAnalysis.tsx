        ...
        <div className="bg-white dark:bg-slate-900 w-full max-w-5xl mx-auto rounded-[50px] overflow-hidden shadow-2xl animate-in fade-in slide-in-from-bottom-10 duration-1000">
            ...
            <div className={cn(
                ...
                {ranking && (
                    <div className="bg-white dark:bg-slate-800 px-10 py-6 rounded-[35px] border border-slate-100 dark:border-slate-700 text-center shadow-xl relative z-10">
                        <span className="block text-[9px] font-black uppercase tracking-[0.3em] text-slate-400 mb-2">Neural Match Score</span>
                        <div className="flex items-center justify-center gap-2">
                            <span className="text-5xl font-black text-blue-600 dark:text-blue-400 tracking-tighter">{ranking.score}<span className="text-2xl opacity-50">%</span></span>
                        </div>
                    </div>
                )}
            </div>
            ...
            {isExtractionFailed && (
                <div className="bg-rose-500 p-8 text-center animate-pulse">
                    <p className="text-white text-lg font-black tracking-tight mb-2">
            ...
            <div className="p-12 grid md:grid-cols-2 gap-12">
                ...
                <div className="space-y-8">
                    <h3 className="text-xl font-black text-slate-900 dark:text-white flex items-center gap-4 tracking-tight">
                    ...
                    <div className="bg-slate-50 dark:bg-slate-800/40 p-10 rounded-[40px] border border-slate-100 dark:border-slate-800 space-y-8 hover:bg-white dark:hover:bg-slate-800 transition-all duration-500 shadow-sm hover:shadow-xl">
                        <div className="flex flex-col border-b border-slate-200 dark:border-slate-700 pb-5">
                            <span className="text-[9px] font-black text-slate-400 uppercase tracking-[0.2em] mb-2">Full Identity</span>
                        ...
                        <div className="space-y-6">
                            <div className="flex items-center gap-4 group">
                                <div className="h-10 w-10 bg-white dark:bg-slate-900 rounded-xl flex items-center justify-center border border-slate-100 dark:border-slate-700 shadow-sm group-hover:bg-blue-600 group-hover:text-white transition-all">
                                    <Mail className="h-4 w-4" />
                                </div>
                                <div className="flex flex-col">
                                    <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest">Email Access</span>
                                    <span className="text-sm font-bold text-slate-700 dark:text-slate-300">{resume.email || "N/A"}</span>
                                </div>
                            </div>

                            <div className="flex items-center gap-4 group">
                                <div className="h-10 w-10 bg-white dark:bg-slate-900 rounded-xl flex items-center justify-center border border-slate-100 dark:border-slate-700 shadow-sm group-hover:bg-emerald-500 group-hover:text-white transition-all">
                                    <Phone className="h-4 w-4" />
                                </div>
                                <div className="flex flex-col">
                                    <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest">Comm Node</span>
                                    <span className="text-sm font-bold text-slate-700 dark:text-slate-300">{resume.phone || "N/A"}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="bg-slate-50 dark:bg-slate-800/40 p-10 rounded-[40px] border border-slate-100 dark:border-slate-800 min-h-[300px] flex flex-wrap gap-3 items-start shadow-sm">
                        {resume.skills && resume.skills.length > 0 ? (
                            resume.skills.map((skill, i) => (
                                <span
                                    key={i}
                                    className="px-5 py-2.5 bg-white dark:bg-slate-900 text-slate-700 dark:text-slate-300 border border-slate-100 dark:border-slate-700 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:border-blue-500/50 hover:scale-110 transition-all cursor-default shadow-sm"
                                >
                                    {skill}
                                </span>
                            ))
                            ...
                            <div className="flex flex-col items-center justify-center w-full h-full text-center py-10">
                                ...
                                <p className="text-slate-400 text-xs font-black uppercase tracking-widest">No Competencies Identified</p>
                            </div>
                        )}
                    </div>

                    <div className="flex items-center gap-3 text-[9px] font-black text-slate-400 uppercase tracking-[0.2em] pl-4">
                            ...
                            <ul className="space-y-6 relative z-10">
                                {resume.experience.map((exp, i) => (
                                    <li key={i} className="flex items-start gap-6 group">
                                        ...
                                        <span className="text-sm font-bold text-slate-600 dark:text-slate-300 leading-relaxed">{exp}</span>
                                    </li>
                                ))}
                            </ul>
                        </div>
            ...
            <div className="bg-slate-900 p-12">
                <div className="flex items-center justify-between mb-8">
                    <h3 className="text-lg font-black text-white flex items-center gap-4 tracking-tight">
                        <FileText className="h-5 w-5 text-slate-500" />
                        Neural Data Stream
                    </h3>
                    <span className="text-[8px] font-black uppercase tracking-[0.4em] text-slate-600">Encrypted Extraction Preview</span>
                </div>
                <div className="bg-black/40 p-8 rounded-3xl text-[10px] text-slate-500 font-mono leading-relaxed h-48 overflow-y-auto custom-scrollbar border border-white/5 shadow-inner">
